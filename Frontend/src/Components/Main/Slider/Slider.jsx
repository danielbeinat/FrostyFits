    ...
    <div className="relative group w-full">
      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            <div className="relative h-full w-full overflow-hidden">
              {/* Gradient Overlay - Menos visible en móvil */}
              <div
                className="absolute inset-0 bg-gradient-to-br opacity-0 md:group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none"
                style={{
                  backgroundImage: `linear-gradient(135deg, ${slide.gradient})`,
                }}
              ></div>
              ...
              <BannerImage
                src={slide.image}
                alt={slide.title}
                className="w-full h-full object-cover object-center md:object-center transform scale-100 md:group-hover:scale-105 transition-transform duration-1000 ease-out"
              />

              {/* Gradients - Ajustados para móvil */}
              <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/50 to-black/30 md:from-black/70 md:via-black/40 md:to-transparent pointer-events-none"></div>
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent pointer-events-none"></div>

              {/* Content overlay - Centrado en móvil, izquierda en desktop */}
              <div className="absolute inset-0 flex items-center justify-center md:justify-start pt-16 md:pt-20">
                <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 md:pl-12 lg:pl-24">
                  <motion.div
                  ...
                  >
                    {/* Badge */}
                    <motion.div
                      className="inline-flex items-center px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-full text-white text-xs sm:text-sm font-semibold mb-3 sm:mb-4 border border-white/20 shadow-lg"
                      initial={{ opacity: 0, x: -30, scale: 0.8 }}
                      animate={{ opacity: 1, x: 0, scale: 1 }}
                      transition={{ duration: 0.6, delay: 0.3, type: "spring" }}
                      whileHover={{
                    ...
                    >
                      <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 text-yellow-300" />
                      {slide.badge}
                    </motion.div>

                    {/* Title - Tamaño responsivo mejorado */}
                    <motion.h1
                      className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-3 sm:mb-4 leading-tight tracking-tight"
                      initial={{ opacity: 0, y: 40 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.8, delay: 0.5, type: "spring" }}
                    >
                    ...
                    {/* Subtitle */}
                    <motion.p
                      className="text-base sm:text-lg md:text-xl lg:text-2xl text-white/95 mb-2 sm:mb-3 font-light tracking-wide px-4 md:px-0"
                      initial={{ opacity: 0, y: 30 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.6, delay: 0.9, type: "spring" }}
                    >
                    ...
                    {/* Description - Oculta en móviles muy pequeños */}
                    <motion.p
                      className="hidden sm:block text-sm sm:text-base md:text-lg text-white/85 mb-6 sm:mb-8 max-w-md mx-auto md:mx-0 leading-relaxed font-light px-4 md:px-0"
                      initial={{ opacity: 0, y: 30 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.6, delay: 1.1, type: "spring" }}
                    >
                    ...
                    {/* CTA Buttons - Stack completo en móvil */}
                    <motion.div
                      className="flex flex-col sm:flex-row gap-3 sm:gap-4 px-4 md:px-0 items-stretch sm:items-center justify-center md:justify-start"
                      initial={{ opacity: 0, y: 30 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.6, delay: 1.3, type: "spring" }}
                    >
                      {/* Primary CTA */}
                      <motion.button
                        className="group relative inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-white text-gray-900 font-bold rounded-xl sm:rounded-2xl shadow-2xl hover:shadow-white/30 transition-all duration-500 text-sm sm:text-base overflow-hidden touch-manipulation"
                        whileHover={{ scale: 1.05, y: -3 }}
                        whileTap={{ scale: 0.95 }}
                        aria-label={`Explorar ${slide.title}`}
                        onClick={() => {
                      ...
                      {/* Secondary CTA */}
                      <motion.button
                        className="inline-flex items-center justify-center px-5 sm:px-6 py-3 sm:py-4 border-2 border-white/40 text-white font-semibold rounded-xl sm:rounded-2xl backdrop-blur-md hover:bg-white/10 hover:border-white/60 transition-all duration-500 text-sm sm:text-base shadow-lg touch-manipulation"
                        whileHover={{ scale: 1.05, y: -3 }}
                        whileTap={{ scale: 0.95 }}
                        aria-label={slide.stats}
                      >
                        <Star className="w-3 h-3 sm:w-4 sm:h-4 mr-2 text-yellow-300 flex-shrink-0" />
                        <span className="whitespace-nowrap">{slide.stats}</span>
                      </motion.button>
                    </motion.div>
                  </motion.div>
                </div>
              ...
              <div className="hidden md:block absolute top-20 right-20 w-40 h-40 bg-white/10 rounded-full blur-3xl animate-pulse pointer-events-none"></div>
              <div className="hidden md:block absolute bottom-32 left-16 w-32 h-32 bg-white/5 rounded-full blur-2xl animate-pulse delay-1000 pointer-events-none"></div>
              <div className="hidden lg:block absolute top-1/2 right-1/3 w-24 h-24 bg-gradient-to-br from-white/20 to-transparent rounded-full blur-xl animate-bounce delay-500 pointer-events-none"></div>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>

      {/* Navigation Arrows - Visible solo en hover en desktop, siempre visible en móvil */}
      <div className="swiper-button-prev-custom absolute left-2 sm:left-4 md:left-6 top-1/2 -translate-y-1/2 z-20 opacity-60 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300">
        <button
          className="p-2 sm:p-3 md:p-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full text-white hover:bg-white/20 hover:scale-110 active:scale-95 transition-all duration-300 shadow-2xl touch-manipulation"
      ...
      <div className="swiper-button-next-custom absolute right-2 sm:right-4 md:right-6 top-1/2 -translate-y-1/2 z-20 opacity-60 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300">
        <button
          className="p-2 sm:p-3 md:p-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full text-white hover:bg-white/20 hover:scale-110 active:scale-95 transition-all duration-300 shadow-2xl touch-manipulation"
      ...
      {/* Progress Bar - Posicionado mejor en móvil */}
